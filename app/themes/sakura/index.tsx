...
import SakuraThemeConfig, { SakuraThemeConfigType } from "./config";
...
export default function SakuraTheme({ data }: Props) {
  const invitationId = "cmgz32v9h000btpk20md1yub7"
  ...
  const [wishes, setWishes] = useState<any[]>([]); // Inisialisasi sebagai array kosong
  ...
  const config = data.config || SakuraThemeConfig;

  const formatGoogleCalendarDate = (dateObj: Date) => {
  ...
  const akadStartDate = new Date(`${config.text.event.akadDate}T${config.text.event.akadStartTime}:00+07:00`);
  const akadEndDate = new Date(`${config.text.event.akadDate}T${config.text.event.akadEndTime}:00+07:00`);
  const receptionStartDate = new Date(`${config.text.event.receptionDate}T${config.text.event.receptionStartTime}:00+07:00`);
  const receptionEndDate = new Date(`${config.text.event.receptionDate}T${config.text.event.receptionEndTime}:00+07:00`);

  const googleCalendarDates = `${formatGoogleCalendarDate(akadStartDate)}/${formatGoogleCalendarDate(akadEndDate)}`;
  ...
  // Fungsi untuk mengambil wishes dari API
  const fetchWishes = async () => {
    if (!invitationId) return; // Jangan fetch jika tidak ada ID
    setIsLoadingWishes(true);
    setWishesError(null);
    try {
      const baseUrl = "https://pestaria-com.vercel.app";
      const response = await fetch(`${baseUrl}/api/wishes?invitationId=${invitationId}`);
      if (!response.ok) {
      ...
      setWishesError(error.message || "Gagal memuat ucapan. Silakan coba lagi.");
    } finally {
      setIsLoadingWishes(false);
    }
  };

  // Panggil fetchWishes saat komponen dimuat atau saat invitationId berubah
  useEffect(() => {
    if (invitationId) {
      fetchWishes();
    }
  ...
  const handleRsvpSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (newRsvp.name && newRsvp.message) {
      if (!invitationId) return alert("Error: Invitation ID is missing.");
      try {
        const rsvpData = {
          ...newRsvp,
          status: newRsvp.attendance,
          guests: newRsvp.attendance === "hadir" ? newRsvp.guests : 0,
        ...
        const { rsvp: submittedWish } = await response.json();
        setWishes((prevWishes) => [submittedWish, ...prevWishes]);
        setNewRsvp({ name: "", message: "", attendance: "hadir", guests: 1 }); // Reset form
      } catch (error) {
  ...
  const coverDisplayDate = receptionStartDate.toLocaleDateString("id-ID", {
    weekday: 'long',
    day: '2-digit',
    month: 'long',
    year: 'numeric',
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row font-serif relative overflow-hidden">
      ...
      <div
        className="relative hidden md:flex flex-1 flex-col items-start justify-end text-left p-8 bg-cover bg-center"
        style={{
      ...
      >
        <div className="absolute inset-0 bg-gradient-to-t from-[#5D6545]/100 via-[#5D6545]/30 to-[#5D6545]/60 z-0"></div>
        <div className="relative z-10">
          <h1 className="text-4xl md:text-5xl font-[Cormorant_Garamond] italic font-semibold mb-3">
            {config.text.couple.groomName} & {config.text.couple.brideName}
          </h1>
          <p className="text-sm tracking-wide opacity-80">{coverDisplayDate}</p>
          <button
            onClick={toggleMusic}
            className="mt-6 w-12 h-12 md:hidden bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center text-white shadow-lg border border-white/20 transition-all hover:scale-110 hover:bg-white/20"
      ...
      <div
        className="relative w-full md:max-w-[425px] h-screen"
        style={{
          ...
          {!isOpened ? (
            ...
            >
              ...
              >
                ...
                <motion.h1
                  ...
                  className="text-5xl mb-6 font-['Great_Vibes'] font-semibold [text-shadow:0_2px_4px_rgba(0,0,0,0.7)]"
                >
            ...
            >
              <div className="min-h-full w-full bg-[#f6f2ef]/90 text-[#484932] text-center">
                  ...
                  <div className="space-y-12">
                    ...
                    >
                      ...
                      <Editable
                        ...
                        className="text-4xl font-['Great_Vibes'] text-[#484932] mb-2"
                      >
                        {data.groom}
                      </Editable>
                      <Editable
                    ...
                    <motion.p
                      className="text-5xl font-['Great_Vibes'] text-[#484932]"
                ...
                <section className="py-20 px-8">
                  <h3 className="text-3xl mb-8 font-['Great_Vibes'] text-[#484932]">
                  ...
                  <div className="grid grid-cols-1 gap-8 max-w-lg mx-auto">
                    <div className="text-center">
                      ...
                      <p className="text-lg font-semibold">
                        {formatDisplayDate(akadStartDate)}
                      </p>
                      <p className="text-sm mt-1">
                        Pukul {formatDisplayTime(akadStartDate)} - {formatDisplayTime(akadEndDate)} WIB
                      </p>
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-lg font-semibold">
                        {formatDisplayDate(receptionStartDate)}
                      </p>
                      <p className="text-sm mt-1">
                        Pukul {formatDisplayTime(receptionStartDate)} - {formatDisplayTime(receptionEndDate)} WIB
                      </p>
                ...
                {config.text.loveStory.display ? (
                  <section className="py-16 px-8 bg-[#f8f4f1]">
                    <h3 className="text-3xl mb-4 font-['Great_Vibes'] text-[#484932]">
                ...
                <section className="py-20 px-8 relative text-white overflow-hidden">
                  ...
                  >
                    ...
                    <Countdown targetDate={akadStartDate} />
                    <a
                      href={`https://www.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent(`The Wedding Of ${config.text.couple.groomFullName} & ${config.text.couple.brideFullName}`)}&dates=${googleCalendarDates}&details=${encodeURIComponent(config.text.closing.paragraph1)}&location=${encodeURIComponent(
                        config.text.event.akadLocation
                      )}`}
                      target="_blank"
                      rel="noopener noreferrer"
                ...
                <section className="py-20 px-8 bg-[#f8f4f1]">
                  <h3 className="text-4xl mb-8 font-['Great_Vibes'] text-[#484932]">
                    Lokasi Acara
                  </h3>
                  <motion.div
                ...
                <section className="py-20 px-8">
                  ...
                  >
                    ...
                    <h3 className="text-4xl mb-4 font-['Great_Vibes'] text-[#484932]">
                    ...
                    <div className="max-w-sm mx-auto space-y-6">
                      {config.text.gift?.bank?.map((b) => (
                        <div className="bg-white/50 border border-[#d9d9c7]/50 rounded-lg p-6 shadow-sm">
                          ...
                          <button
                            onClick={() =>
                              handleCopy(b.accountNumber, "rekening")
                            }
                      ...
                      <div className="bg-white/50 border border-[#d9d9c7]/50 rounded-lg p-6 shadow-sm">
                        ...
                        <button
                          onClick={() =>
                            handleCopy(config.text.gift.addressDetail, "alamat")
                          }
                          className="inline-flex items-center gap-2 mt-4 text-xs bg-[#484932] text-white font-semibold px-3 py-1.5 rounded-full hover:bg-[#3a3b29] transition-colors"
                        >
                          {copiedText === "alamat" ? (
                ...
                <section className="py-16 px-8 bg-[#484932] text-[#F8F7F5]">
                  ...
                  >
                      ...
                      <div className="flex gap-4">
                        <label className="flex items-center gap-2">
                          <input
                            ...
                            name="attendance"
                            value="hadir"
                            checked={newRsvp.attendance === "hadir"}
                            onChange={(e) =>
                              setNewRsvp({
                                ...newRsvp,
                        ...
                        <label className="flex items-center gap-2">
                          <input
                            ...
                            name="attendance"
                            value="tidak"
                            checked={newRsvp.attendance === "tidak"}
                            onChange={(e) =>
                              setNewRsvp({
                                ...newRsvp,
                                attendance: e.target.value,
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewRsvp({ ...newRsvp, message: e.target.value })
                        }
                        className="w-full bg-white/50 border border-[#d9d9c7] rounded-md px-3 py-2 focus:ring-[#484932] focus:border-[#484932] transition-colors"
                  ...
                  {isLoadingWishes ? (
                    <p className="text-center text-gray-300">Memuat ucapan...</p>
                  ) : wishesError ? (
                    <p className="text-center text-red-400">{wishesError}</p>
                  ) : (
                ...
                <section className="py-20 px-8">
                  ...
                  <p className="mt-4 font-['Great_Vibes'] text-5xl text-[#484932]">
                    {config.text.couple.groomName} &{" "}
                    {config.text.couple.brideName}
                  </p>
                </section>
